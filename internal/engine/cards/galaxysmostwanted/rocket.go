...
func registerRocketCards() {
	...
	engine.RegisterBehavior("16019", &engine.Behavior{
		React: func(g *engine.Game, e engine.Entity, msg engine.Message) []engine.Message {
			...
			g.TLogf("c.rocketRaccoon3AtkAgainst", cardutil.EnemyLabel(g.Entity(w.Target)))
	...
	engine.RegisterBehavior("16030", &engine.Behavior{
		...
		React: func(g *engine.Game, e engine.Entity, msg engine.Message) []engine.Message {
			...
			g.TLogf("c.iVeGotAPlanRocketRaccoonReadies")
				...
				if c.Code[:5] == "16033" {
					...
					if len(picks) > 0 {
						return []engine.Message{engine.AskQuestion{Player: p.ID,
							Question: engine.Ask(engine.Tf("c.salvagePutWhichTechUpgradeOnTopOfYourDeck"), picks...)}}
					}
					return nil
				}
			}
	...
	engine.RegisterBehavior("16034", &engine.Behavior{
		...
		Abilities: func(g *engine.Game, e engine.Entity) []engine.Ability {
			...
			return []engine.Ability{{
				Label: engine.Tf("c.exhaustBatteryPackMove1ChargeToATechUpgrade"), Type: engine.AbilityAction,
				Exhaust: true,
				Execute: func(g *engine.Game, self engine.EntityID) []engine.Message {
					...
					return []engine.Message{engine.AskQuestion{Player: p.ID,
						Question: engine.Ask(engine.Tf("c.batteryPackAdd1ChargeToWhichTechUpgrade"), targets...)}}
				},
			}}
		},
	})
	...
	engine.RegisterBehavior("16035", &engine.Behavior{
		OnPlay: func(g *engine.Game, e engine.Entity) []engine.Message {
			...
			if p != nil {
				p.MaxHP += 3
				g.TLogf("c.gets3HitPoints", p.Name)
			}
			return nil
		},
	...
	engine.RegisterBehavior("16037", &engine.Behavior{
		...
		Abilities: func(g *engine.Game, e engine.Entity) []engine.Ability {
			...
			return []engine.Ability{{
				Label: engine.Tf("c.exhaustRocketLauncher1Charge2DamageToTheVillainAndAPlayerSMi"), Type: engine.AbilityAction,
				...
				Execute: func(g *engine.Game, self engine.EntityID) []engine.Message {
					...
					return []engine.Message{engine.AskQuestion{Player: owner.ID,
						Question: engine.Ask(engine.Tf("c.rocketLauncherHitTheVillainAndWhichPlayerSMinions"), picks...)}}
				},
			}}
		},
	})
...
func chargeCannonAbility(code, prompt string, dmg int) func(g *engine.Game, e engine.Entity) []engine.Ability {
	return func(g *engine.Game, e engine.Entity) []engine.Ability {
		...
		return []engine.Ability{{
			Label:    engine.S("Exhaust + 1 charge → deal " + itoa(dmg) + " damage (overkill, ranged)"),
			...
			Execute: func(g *engine.Game, self engine.EntityID) []engine.Message {
				...
				return cardutil.ChooseEnemy(engine.S(prompt),
...
func registerRocketBasics() {
	...
	floraFauna := &engine.Behavior{
		OnPlay: func(g *engine.Game, e engine.Entity) []engine.Message {
			...
			if p.HeroCode[:5] == "16001" {
				addGrowth(p, 2)
				picks = append(picks, engine.Choice{
					ID: "groot", Label: engine.Tf("c.place2GrowthCountersOnGrootAndReadyHim"), Kind: engine.ChoiceLabel,
			...
			return []engine.Message{engine.AskQuestion{Player: p.ID,
				Question: engine.Ask(engine.Tf("c.floraAndFaunaWhichMode"), picks...)}}
		},
	}
	...
	engine.RegisterBehavior("16046", &engine.Behavior{
		...
		React: func(g *engine.Game, e engine.Entity, msg engine.Message) []engine.Message {
			...
			u.Exhausted = true
			u.Counters--
			g.TLogf("c.handCannon2DamageForThisAttack")
	...
	engine.RegisterBehavior("16052", &engine.Behavior{
		DamagePrevention: func(g *engine.Game, u *engine.Upgrade, p *engine.Player, n int) (int, int) {
			...
			g.TLogf("c.boosterBoots1DamagePrevented")
			return 1, 0
		},
	})
}
	...
	engine.RegisterBehavior("16053", &engine.Behavior{
		ResolveObligation: func(g *engine.Game, p *engine.Player, card engine.Card) []engine.Message {
			exhaust := engine.Choice{
				ID: "exhaust", Label: engine.Tf("c.exhaustYourAlterEgoRemoveCrisisOnHalfworldFromTheGame"), Kind: engine.ChoiceLabel,
			}.Msgs(
			...
			if best >= 0 {
				strip = append(strip, engine.Choice{
					ID: "strip", Label: engine.Tf("c.discardYourHighestCostUpgrade"), Kind: engine.ChoiceLabel,
				}.Msgs(engine.DiscardControlled{Player: p.ID, ID: p.Upgrades[best]},
					engine.ObligationResolve{Player: p.ID, Card: card}))
			} else {
				strip = append(strip, engine.Choice{
					ID: "surge", Label: engine.Tf("c.noUpgradesToDiscardSurge"), Kind: engine.ChoiceLabel, Disabled: true,
				})
			}
			return []engine.Message{engine.AskQuestion{Player: p.ID,
				Question: engine.Ask(engine.Tf("c.crisisOnHalfworldChooseAnOption"), strip...)}}
		},
	})
}
...
func registerRocketNemesis() {
	...
	engine.RegisterBehavior("16056", &engine.Behavior{
		OnAttach: func(g *engine.Game, t *engine.Attachment, target engine.EntityID) []engine.Message {
			...
			for id := range g.Minions {
				if m := g.Minions[id]; m != nil && m.Code[:5] == "16055" {
					t.Target = id
					g.TLogf("c.blackjackSBazookaAttachesToBlackjackOHare")
					return nil
				}
			}
			for id := range g.Villains {
				t.Target = id
				g.TLogf("c.blackjackSBazookaAttachesToTheVillain")
				return nil
			}
			return nil
		},
		Abilities: func(g *engine.Game, e engine.Entity) []engine.Ability {
			return []engine.Ability{{
				Label: engine.Tf("c.spend3MentalResourcesDiscardBlackjackSBazooka"), Type: engine.AbilityAction,
				Execute: func(g *engine.Game, self engine.EntityID) []engine.Message {
